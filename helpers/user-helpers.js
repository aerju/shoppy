...
module.exports = {
    doSignup: (userData) => {
        ...
        return new Promise(async (res, rej) => {
            ...
            db.get().collection(collection.USER_INFORMATION).insertOne(userData).then((data) => {
    ...
    doLogin: (userData) => {
        return new Promise(async (res, rej) => {
            ...
            let response = {}
            let user = await db.get().collection(collection.USER_INFORMATION).findOne({ email: userData.email })
            if (user.userstatus) {
        ...
        return new Promise(async (reslove, reject) => {
            let user = await db.get().collection(collection.USER_INFORMATION).findOne({ _id: objectId(userId) })
    ...
    doOtplogin: (number) => {
        ...
        return new Promise(async (resolve, reject) => {
            let getNumber = await db.get().collection(collection.USER_INFORMATION).findOne({ phone: number })
            if (getNumber) {
                response.status = true
                resolve(getNumber)
        ...
        return new Promise(async (resolve, reject) => {
            ...
            db.get().collection(collection.USER_INFORMATION).updateOne({ _id: userId }, {
                $set: {
                    password: pass
                }
            }).then((response) => {
    ...
    gererateRazopay: (orderId, total) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            hmac.update(orderDeatails['payment[razorpay_order_id]'] + '|' + orderDeatails['payment[razorpay_payment_id]']);
    ...
    changePaymentStatus: (orderId) => {
        return new Promise((resolve, reject) => {
            db.get().collection(collection.ORDER_INFORMATION).updateOne({ _id: objectId(orderId) },
    ...
    getAddressInfo: (userId) => {
        return new Promise(async (resolve, reject) => {
            let addAddresdetails = await db.get().collection(collection.USER_INFORMATION).findOne({ _id: objectId(userId) })
        ...
        return new Promise(async (resolve, reject) => {
            let checKCoupone = await db.get().collection(collection.COUPONE_MANAGEMENT).findOne({ coupone_code: coupone.couponeCode })
            usedUsers = []
            if (checKCoupone) {
                let couponeId = checKCoupone._id
                const couponeUsed = checKCoupone.usedUsers?.findIndex(obj => obj.user === objectId(userId));

                let couponeDetails = await db.get().collection(collection.COUPONE_MANAGEMENT).find({ _id: objectId(couponeId) }).toArray()
                ...
                if (couponeUsed) {
                    ...
                    db.get().collection(collection.COUPONE_MANAGEMENT).updateOne({ _id: objectId(couponeId) }, {
                        $push: {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const address = await db.get().collection(collection.USER_INFORMATION)
                .findOne(
                    {
                        _id: userId,
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let address = await db.get().collection(collection.USER_INFORMATION)
                .findOne(
                    {
                        _id: objectId(userId),
        ...
        return new Promise(async (resolve, reject) => {
            ...
            db.get().collection(collection.USER_INFORMATION)
                .update(
                    {

                        address: { $elemMatch: { _id: objectId(addressDetails.addressId) } }
                    },                    
    ...
    getWalletInfo: (userId) => {
        return new Promise(async (resolve, reject) => {
            let wallet = await db.get().collection(collection.ORDER_INFORMATION).aggregate([
                {
                    $match: { userId: objectId(userId), orderStatus: 'Cancel' , paymentMethod: 'razorpay' }
                },  
                {
                    $group: {
                        _id: null,
