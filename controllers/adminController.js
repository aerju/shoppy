...
module.exports = {
    ...
    adminHome: async (req, res) => {
        ...
        // let data=res.json(OrderStatistics)
    ...
    adminViewAllCategories: (req, res) => {
        ...
        // console.log(req.body);
        adminHelper.editCategoryPost(req.params.id, req.body).then((findCategory) => {
            if (findCategory) {
                message = "Category Already Exist"
    ...
    adminAddProduct: (req, res) => {
        ...
        adminHelper.viewAllCategories().then((category) => {
            ...
            res.render('admin/addProduct', { category, 'submitStatus': req.session.submitStatus });
    ...
    adminAddProductPost: async (req, res) => {
        ...
        try {
            console.log(req.files, 'llllllllllllllllllllllllllllllllllllllllllllllllllllllllllllll')
            const imgUrl = [];
    ...
    adminEditProductPost: async (req, res) => {
        ...
        try {
            ...
            adminHelper.editProduct(req.params.id, req.body).then(() => {
                if (imgUrl.length != 0) {
                    adminHelper.addProductImages(req.params.id, imgUrl).then((response) => {
                        console.log(response);
                    })
    ...
    salesReport: async (req, res) => {
        ...
        for (let i = 0; i < salesReport.length; i++) {
            salesReport[i].date = salesReport[i].date.getDate() + '-' + months[salesReport[i].date.getMonth()] + '-' + salesReport[i].date.getFullYear();
        }
        ...
        let saleStatistics = await adminHelper.getSaleStatistics(year)
        ...
        res.json({ OrderStatistics, saleStatistics, categortStatics, totalProducts })
